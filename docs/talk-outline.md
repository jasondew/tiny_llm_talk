...
| 3 | Attention | 11 | **dot product** with two arrows to drag, **softmax playground**, **fuzzy map**, ask/offer/hand over, one head of attention (the formula over the code, sixteen lines), three details with a **mask toggle**, **bet: where will the blank look**, **the walkthrough** | presenter drags, slider, picks; audience bet |

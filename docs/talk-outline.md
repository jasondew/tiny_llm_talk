...
| 3 | Attention | 11 | **softmax playground**, ask/offer/hand over, one head of attention (the formula over the code, sixteen lines), **dot product** with two arrows to drag, **fuzzy map**, three details with a **mask toggle**, **bet: where will the blank look**, **the walkthrough** | presenter drags, slider, picks; audience bet |

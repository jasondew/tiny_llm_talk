...
| 4 | The rest of the block | 4 | the transformer in this talk (the block diagram again, × 1, the box named), normalization, the neural network as a node graph, and the ReLU with its graph, each with the dogs row, then Block.forward stepped down its nine lines with the flow and its two skips lit beside it | |

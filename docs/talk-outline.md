...
| --- | --- | --- | --- | --- |
| 0 | Cold open | 4 | **vote: flees, or flee** (the sentence and the join link, on screen while the room arrives), title, what a transformer is (one block, drawn; every frontier model is one; this one from nothing), the parameters (fifteen thousand floats, counted stage by stage, every one learned) | audience vote |
...
| 3 | Attention, from `Map` | 11 | `Map.get`, **dot product** with two arrows to drag, **softmax playground**, **fuzzy map**, learn the lookup, the code, three details with a **mask toggle**, **bet: where will the blank look**, **the walkthrough** | presenter drags, slider, picks; audience bet |

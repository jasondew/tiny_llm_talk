...
| 3 | Attention, from `Map` | 11 | one head of attention (the formula over the code, sixteen lines), **dot product** with two arrows to drag, **softmax playground**, **fuzzy map**, learn the lookup, three details with a **mask toggle**, **bet: where will the blank look**, **the walkthrough** | presenter drags, slider, picks; audience bet |

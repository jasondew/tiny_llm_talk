...
| 4 | The rest of the block | 4 | the transformer in this talk (the block diagram again, × 1, the box named), normalization and the neural network, each with the dogs row, then Block.forward stepped down its nine lines with the flow and its two skips lit beside it | |
